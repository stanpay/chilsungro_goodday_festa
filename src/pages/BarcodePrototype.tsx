...
const BarcodePrototype = () => {
    ...
    // ========== 바코드 표시 컴포넌트 ==========
    const BarcodeDisplay = ({ number, bare, kiosk }: {
        number: string;
        bare?: boolean;
        kiosk?: boolean;
    }) => {
        ...
        if (kiosk) {
            return (<div className="flex flex-col w-full items-stretch">
        ...
        {bare ? (<svg ref={svgRef} className="max-w-full h-20" style={{ maxHeight: '80px' }}/>) : (<div className="flex items-center justify-center bg-white p-2 rounded-lg">
    ...
    const kioskCards = useMemo(() => [
        {
            id: 0,
            content: (<div className="flex flex-col w-full h-full p-4 min-w-0" onClick={(e) => e.stopPropagation()}>
            <div className="flex flex-col items-start w-full flex-1 min-h-0">
              <BarcodeDisplay number={barcodeNumber} kiosk/>
            </div>
            <div className="bg-white p-4 space-y-1 shrink-0" style={{ color: "rgba(38, 34, 42, 1)", backgroundColor: "rgba(255, 255, 255, 1)" }}>
    ...
    return (<div className="bg-background h-screen overflow-hidden">
      {/* 키오스크 모드 오버레이 */}
      {isKioskMode && (<div className="fixed inset-0 z-50 bg-background flex flex-col pt-0 px-0 pb-4 overflow-hidden" onClick={exitKioskMode} role="button" tabIndex={0} onKeyDown={(e) => e.key === "Escape" && exitKioskMode()} aria-label="바코드가 아닌 영역 터치 시 키오스크 모드 종료">
          <div className="flex-1 min-h-0 flex flex-col items-center overflow-hidden pt-0 pb-2" onClick={(e) => e.stopPropagation()}>
            <CardStack items={kioskCards} initialIndex={currentCardIndex} onIndexChange={setKioskCurrentIndex} offset={40} scaleFactor={0.08} className="relative w-full h-[320px] shrink-0" cardClassName="absolute left-0 right-0 top-0 rounded-t-none rounded-b-2xl border border-border/50 bg-card shadow-lg overflow-hidden flex flex-col h-[280px] w-full"/>
            {/* 위로 스와이프 안내 - 바코드 스택 바로 아래 */}
            <div className="flex flex-col items-center gap-2 pt-2 shrink-0 px-4">
              <div className="flex items-center gap-1.5 text-muted-foreground">
                <ChevronUp className="w-5 h-5 animate-bounce" style={{ animationDuration: "1.5s" }} aria-hidden/>
      ...
      {/* 헤더 - 키오스크 모드에서는 숨김 */}
      {!isKioskMode && (<div className="absolute left-2 top-4 flex flex-col gap-3 z-50">
          {/* 페이지 인디케이터 */}
      ...
      <div className="flex flex-col h-full py-4 overflow-hidden">
        <div className="flex-1 overflow-hidden snap-y snap-mandatory scrollbar-hide pb-32 px-4" style={{
          ...
          {/* 기프티콘 바코드 카드 */}
          <div className="snap-start mb-4" style={{
            scrollSnapAlign: 'start',
            scrollSnapStop: 'always',
        }}>
            <Card className="p-0 rounded-2xl border-border/50 relative overflow-hidden cursor-pointer" onClick={enterKioskMode}>
              ...
              {/* 기프티콘 정보 영역 */}
              <div className="rounded-b-2xl bg-white border border-border/50 p-4 space-y-1" style={{
          ...
          <div className="snap-start" style={{
            ...
            <Card className="p-0 rounded-2xl border-border/50 relative overflow-hidden cursor-pointer" onClick={enterKioskMode}>
              ...
              <div className="rounded-t-2xl h-[130px] bg-muted border border-border/50 border-b-0 px-4 pt-3 pb-1.5 relative flex flex-col items-center justify-center gap-2">
                ...
                <button type="button" className="w-12 h-12 rounded-full border-2 border-dashed border-primary flex items-center justify-center text-primary hover:border-primary/80 hover:text-primary/80 transition-colors" onClick={(e) => {
            e.stopPropagation();
        }}>
            ...
            backgroundColor: "rgba(255, 255, 255, 1)",
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button size="sm" className="shrink-0 h-8 text-xs font-medium gap-1.5 bg-primary text-primary-foreground hover:bg-primary/90" onClick={(e) => {
            e.stopPropagation();
        }}>
      ...
      <Button onClick={isKioskMode ? exitKioskMode : enterKioskMode} variant="outline" className="fixed bottom-40 right-6 h-14 w-14 rounded-full shadow-lg z-50 bg-background border-2 border-primary text-primary hover:bg-primary/10" size="icon" title={isKioskMode ? "일반 모드" : "키오스크 모드"}>
