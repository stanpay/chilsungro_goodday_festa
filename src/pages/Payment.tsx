...
import { DUMMY_FRANCHISES, DUMMY_FRANCHISE_PAYMENT_METHODS, DUMMY_STORES, DUMMY_PAYMENT_HISTORY, DUMMY_USED_GIFTICONS, } from "@/api/dummyData";
...
class _QB {
    ...
    private async _exec() {
        ...
        let rows = this._rows();
        if (this._lim < Infinity)
            rows = rows.slice(0, this._lim);
        if (this._sngl)
    ...
    private _rows(): any[] {
        switch (this._t) {
            ...
            case "stores": {
                if (this._eq["id"])
                    return DUMMY_STORES.filter((s) => s.id === this._eq["id"]);
                if (this._eq["franchise_id"])
                    return DUMMY_STORES.filter((s) => s.franchise_id === this._eq["franchise_id"]);
                return [...DUMMY_STORES];
            }
            ...
            case "used_gifticons": {
                ...
                if (this._eq["available_at"])
                    r = r.filter((g) => g.available_at === this._eq["available_at"]);
                if (this._eq["status"])
                    r = r.filter((g) => g.status === this._eq["status"]);
                if (this._eq["reserved_by"])
                    r = r.filter((g) => (g as any).reserved_by === this._eq["reserved_by"]);
                if (this._in["id"])
                    r = r.filter((g) => this._in["id"].includes(g.id));
                return r;
            }
            default: return [];
        }
...
const Payment = () => {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const purchasedGifticonsData: Array<{
            ...
            type UsedGifticonWithName = UsedGifticon & {
                name?: string;
            };
      ...
      {(step === 1 || step === 2) && (<header className="sticky top-0 z-50 bg-card border-b border-border">
          <div className="max-w-md mx-auto py-4 relative">
            {step === 2 ? (<button onClick={() => {
              ...
              </button>) : (<Link to="/main" className="absolute left-4 top-1/2 -translate-y-1/2 z-10">
                <Button variant="ghost" size="icon" className="rounded-full">
                  <ArrowLeft className="w-5 h-5"/>
                </Button>
              </Link>)}
        ...
        {step === 1 ? (<>
            ...
            {canUseGifticon && (<>
                ...
                {(isAutoSelectMode ? autoSelectedGifticons : gifticons).length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                        ...
                        return (<div key={gifticon.id} className={`p-4 rounded-xl transition-all ${isDisabled && !isSelected
                ...
                {selectedGifticons.size > 0 && (<div className="mt-4 pt-4 border-t border-border space-y-4">
                    {/* 할인쿠폰 선택 섹션 */}
            ...
            {/* Confirm Button */}
            <Button onClick={handleConfirmStep1} className="w-full h-14 text-lg font-semibold rounded-xl" disabled={isLoading}>
              확인
            </Button>
            ...
            <div className="space-y-4">
              ...
              {/* 결제하기 버튼 */}
              <Button id="payment-button" onClick={async () => {
                if (!paymentWidgets) {
            ...
            <div className="flex flex-col h-full py-4 overflow-hidden">
              <div className="flex-1 overflow-y-auto snap-y snap-mandatory scrollbar-hide pb-32" style={{
            ...
            <div className="absolute bottom-4 left-4 right-4 space-y-3">
              <div className="relative">
                ...
                <Button ref={payAppButtonRef} onClick={handlePayWithNaverPay} className="w-full h-14 text-lg font-semibold rounded-xl opacity-50" disabled={true}>
