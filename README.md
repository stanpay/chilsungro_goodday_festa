...
- **`VITE_*` (클라이언트)** — 빌드 시 번들에 인라인되므로 **비밀키를 넣지 마세요.**
  네이버 지도 clientId와 카카오 JavaScript 키는 도메인 화이트리스트로 보호되는 공개 키라 노출이 정상입니다.
...
빌드·타입 검사·린트를 실행합니다. 현재 타입 검사와 린트는 기존 위반이 남아 있어
비블로킹으로 설정되어 있으며, 백로그 정리 후 블로킹으로 승격할 예정입니다.
