...
const SellResult = () => {
    ...
    const adjustPrice = (itemId: string, percentage: number) => {
        setGifticonItems((items) => items.map((item) => {
            ...
            if (jobId)
                localStorage.setItem(`gifticon_price_${jobId}`, newPrice.toString());
            return { ...item, currentPrice: newPrice };
        }));
    };
    const handleConfirm = async () => {
        if (!job?.recognition_result || !user)
            return;
        try {
    ...
    const PriceAdjustButtons = ({ itemId }: {
        itemId: string;
    }) => (<div className="flex gap-0.5">
      {([-5, -1, 1, 5] as const).map((pct) => (<Button key={pct} variant="outline" size="sm" className="h-8 px-2.5 text-xs flex-1" onClick={(e) => { e.stopPropagation(); adjustPrice(itemId, pct); }}>
    ...
    return (<div className="min-h-screen bg-background pb-20">
      ...
      <main className="max-w-md mx-auto px-4 py-6 space-y-4">
        {failedItems.map((item) => (<Card key={item.id} className="p-4 border-2 border-destructive relative">
            <Button variant="ghost" size="icon" className="absolute top-2 right-2 h-6 w-6" onClick={() => handleRemoveItem(item.id)}>
              <X className="w-4 h-4"/>
            </Button>
            <div className="flex items-start gap-3">
              {imageData && (<div className="w-20 h-20 shrink-0 rounded-lg overflow-hidden bg-muted">
        ...
        {partialFailedItems.map((item) => (<Card key={item.id} className="p-4 pt-10 border-2 border-yellow-500 relative cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate(`/sell/result/${jobId}/detail`)}>
            <div className="absolute top-2 left-1/2 -translate-x-1/2 z-10">
              <p className="text-xs font-semibold text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded">일부 인식 실패 직접 입력해주세요</p>
            </div>
            <Button variant="ghost" size="icon" className="absolute top-2 right-2 h-6 w-6 z-10" onClick={(e) => { e.stopPropagation(); handleRemoveItem(item.id); }}>
              <X className="w-4 h-4"/>
            </Button>
            <div className="flex items-start gap-3 pr-8">
              ...
              <div className="flex-1 min-w-0 flex flex-col">
                <div className="flex items-center justify-between mb-1">
                  <AutoFitMarquee as="h3" text={item.productName} className="flex-1" textClassName="font-semibold" fontSizeClasses={["text-sm", "text-xs"]}/>
                  <div className="flex items-center gap-2 shrink-0 ml-2" onClick={(e) => e.stopPropagation()}>
                    <span className="text-xs text-muted-foreground">판매</span>
                    <Switch checked={item.isSelling} onCheckedChange={(checked) => setGifticonItems((items) => items.map((i) => i.id === item.id ? { ...i, isSelling: checked } : i))}/>
                  </div>
                </div>
        ...
        {successItems.map((item) => (<Card key={item.id} className="p-4 cursor-pointer hover:shadow-md transition-shadow relative" onClick={() => navigate(`/sell/result/${jobId}/detail`)}>
            <div className="flex items-start gap-3 pr-8">
              {imageData && (<div className="w-20 h-20 shrink-0 rounded-lg overflow-hidden bg-muted">
                  <img src={imageData} alt="기프티콘" className="w-full h-full object-cover"/>
                </div>)}
        ...
        {(successItems.length > 0 || partialFailedItems.length > 0) && (<div className="flex flex-col gap-3 pt-4">
            {hasFailedItems && (<div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3 mb-2">
