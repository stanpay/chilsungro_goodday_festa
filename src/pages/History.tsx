      ...
      <div className="max-w-md mx-auto px-4 py-4 space-y-3 border-b border-border">
        ...
        <div className="flex gap-2">
          ...
          {(dateRange.from || searchQuery) && (
            <Button variant="ghost" onClick={() => { setDateRange({}); setSearchQuery(""); }}>초기화</Button>
          )}
        </div>
      </div>
      ...
      <main className="max-w-md mx-auto px-4 py-6">
        <div className="space-y-6">
          {Object.keys(groupedPayments).length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">결제 내역이 없습니다</div>
          ) : (
            Object.entries(groupedPayments).map(([date, datePayments]) => (
              <div key={date}>
                <h2 className="text-sm font-semibold text-muted-foreground mb-3">{formatDateLabel(date)}</h2>
                <div className="space-y-3">
                  {datePayments.map((payment) => (
                    <Card key={payment.id} className="p-4 rounded-xl border-border/50">
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          <h3 className="font-bold text-lg mb-1">{payment.store}</h3>
                          <p className="text-sm text-muted-foreground mb-2">
                            {format(new Date(`2000-01-01 ${payment.time}`), "HH:mm")}
                          </p>
                          <p className="text-sm text-muted-foreground">{payment.method}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-primary font-bold text-lg mb-2">{payment.amount.toLocaleString()}원</p>
                          <Badge variant={payment.status === "완료" ? "default" : "secondary"}>{payment.status}</Badge>
                        </div>
                      </div>
                    </Card>
                  ))}
