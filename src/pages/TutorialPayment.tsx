...
const TutorialPayment = () => {
    ...
    */
    ...
    useEffect(() => {
        if (!(step === 1 && showPaymentGuide && selectedGifticons.size > 0))
            return;
                ...
                setGifticons(prev => {
                    const remaining = prev.filter(g => {
                        ...
                        if (initialGifticonIds.has(g.id)) {
                            ...
                            if (gifticonsToRemove.includes(g.id))
                                return false;
                            return true;
                        }
                        // 제거 대상 추가 기프티콘만 제거
    ...
    const couponDiscount = useMemo(() => {
        if (!selectedCoupon || totalCostBeforeCoupon === 0)
            return 0;
        // 최소 구매 금액 체크
    ...
    // 선택한 기프티콘 목록 생성
    const purchasedGifticonsList: Array<{
        id: string;
    ...
    useEffect(() => {
        const fetchActualBarcodes = async () => {
            if (step !== 2 || selectedGifticons.size === 0)
                return;
            // 모든 예약된 기프티콘 ID 수집
    ...
    useEffect(() => {
        const markGifticonsAsUsed = async () => {
            if (step !== 2 || recentlyPurchasedCount === 0)
                return;
            if (!isLoggedIn)
                return;
            const { data: { session } } = await supabase.auth.getSession();
    ...
    return (<div className={`bg-background ${step === 3 ? 'h-screen overflow-hidden' : 'min-h-screen pb-6'}`}>
      {/* 튜토리얼 배너 (튜토리얼 모드에서만 표시) */}
      {isTutorial && <TutorialBanner />}
      
      {(step === 1 || step === 2) && (<header className="sticky top-0 z-50 bg-card border-b border-border">
          <div className="max-w-md mx-auto py-4 relative">
            {step === 2 ? (<button onClick={() => {
              ...
              </button>) : (<Link to={isTutorial ? "/tutorial" : "/main"} className="absolute left-4 top-1/2 -translate-y-1/2 z-10">
                <Button variant="ghost" size="icon" className="rounded-full">
                  <ArrowLeft className="w-5 h-5"/>
                </Button>
              </Link>)}
            <div className="flex flex-col items-center justify-center">
              ...
              {step === 1 && isLoggedIn && storeBrand && (<div className="text-xs text-muted-foreground mt-1">
      ...
      <main className={`max-w-md mx-auto ${step === 3 ? 'h-full flex flex-col pl-14 pr-4 overflow-hidden' : 'px-4 py-6 space-y-4'}`}>
        {step === 1 ? (<>
            ...
            <div className={`space-y-3 ${isTutorial ? 'pointer-events-none opacity-50' : ''}`}>
              ...
              {paymentMethods.length === 0 ? (<div className="flex flex-col items-center justify-center py-12">
                ...
                return (<Card key={method.id} className={`p-4 transition-all border-2 ${!isEnabled
                        ? "bg-muted/30 border-muted opacity-60 cursor-not-allowed"
                        : isSelected
                            ? "border-primary bg-primary/5 cursor-pointer"
                            : "border-border/50 hover:border-border cursor-pointer"}`} onClick={() => {
                        if (isEnabled) {
                    ...
                    <div className="relative">
                      {!isEnabled && (<div className="absolute inset-0 flex items-center justify-center z-10">
                      ...
                      <div className={`flex items-start justify-between ${!isEnabled ? 'opacity-50' : ''}`}>
                        ...
                        setSelectedPaymentOptions(newSet);
                    }} disabled={!isEnabled} className="mt-1"/>
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <h3 className={`font-semibold text-sm ${!isEnabled ? 'text-muted-foreground' : ''}`}>
                                {method.name}
                              </h3>
                              {isRecommended && (<Badge variant="secondary" className="bg-primary/10 text-primary border-none text-[10px] px-1.5 py-0 h-4">
                                  추천
                                </Badge>)}
                            </div>
                            {displayDiscount && (<p className="text-xs text-muted-foreground mt-1">
                                {displayDiscount}
                              </p>)}
                          </div>
                        </div>
            ...
            {canUseGifticon && (<>
                ...
                <div className={`space-y-2 mb-4 ${isTutorial ? 'pointer-events-none opacity-50' : ''}`}>
                  ...
                  <div className="flex gap-2 items-center">
                    <div className="flex-1 relative">
                      <Input type="text" placeholder="금액을 입력하세요 (원)" value={inputBudget !== null ? formatNumberWithCommas(inputBudget) : ""} onChange={(e) => {
                ...
                }} className="w-full pr-24" min="0" max="999999"/>
                      {inputBudget !== null && inputBudget > 0 && (<span className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm pointer-events-none whitespace-nowrap">
                          {formatToKoreanCurrency(inputBudget)}
                        </span>)}
                    </div>
                    {!isAutoSelectMode ? (<Button onClick={executeAutoSelect} disabled={!inputBudget || inputBudget <= 0 || isLoading} className="shrink-0">
                        확인
                      </Button>) : (<>
                        <Button onClick={() => {
                        ...
                        executeAutoSelect();
                    }} disabled={!inputBudget || inputBudget <= 0 || isLoading} className="shrink-0">
                          수정
                        </Button>
                        <Button onClick={cancelAutoSelect} variant="outline" className="shrink-0">
                          취소
                        </Button>
                      </>)}
                  </div>
                ...
                {(isAutoSelectMode ? autoSelectedGifticons : gifticons).length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                        등록된 기프티콘이 없습니다.
                      </div>) : (<>
                        ...
                        return (<div key={gifticon.id} ref={isTutorial && index === 0 ? gifticonGuideRef : null} className={`p-4 rounded-xl transition-all ${isDisabled && !isSelected
                                ...
                                : isSelected
                                    ...
                                    : "bg-muted/50 border border-transparent hover:border-border cursor-pointer"}`} onClick={() => {
                                if (!isDisabled) {
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center">
                                  <Checkbox checked={isSelected} disabled={isDisabled} className="w-5 h-5"/>
                                </div>
                              </div>
                            </div>);
                    })}
                ...
                {selectedGifticons.size > 0 && (<div className={`mt-4 pt-4 border-t border-border space-y-4 ${isTutorial ? 'pointer-events-none opacity-50' : ''}`}>
                    {/* 할인쿠폰 선택 섹션 */}
                    {availableCoupons.length > 0 && (<Collapsible open={isCouponExpanded} onOpenChange={setIsCouponExpanded}>
                        <div className="space-y-2">
                          <CollapsibleTrigger className="w-full">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                {selectedCoupon && couponDiscount > 0 && !isCouponExpanded && (<span className="text-xs text-primary font-medium">
                          ...
                          {!isCouponExpanded && selectedCoupon && couponDiscount > 0 && (<div className="p-3 rounded-lg border border-primary/20 bg-primary/5">
                              <div className="flex items-center justify-between">
                                <div>
                          ...
                          <CollapsibleContent>
                            ...
                            return (<div key={coupon.id} className={`p-3 rounded-lg border-2 transition-all cursor-pointer ${isSelected
                                    ? 'border-primary bg-primary/5'
                                    : canUse
                                        ? 'border-border/50 hover:border-border'
                                        : 'border-muted/50 opacity-50 cursor-not-allowed'}`} onClick={() => {
                                    if (canUse) {
                                    ...
                                    <div className="flex items-center justify-between">
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2">
                                          <Checkbox checked={isSelected} disabled={!canUse} className="w-4 h-4"/>
                                          <div>
                                            <p className="font-semibold text-sm">{coupon.name}</p>
                                            <p className="text-xs text-muted-foreground mt-0.5">
                                              {coupon.description}
                                            </p>
                                            {coupon.min_purchase_amount && (<p className="text-xs text-muted-foreground mt-0.5">
                                                최소 {coupon.min_purchase_amount.toLocaleString()}원 이상 구매
                                              </p>)}
                                          </div>
                                        </div>
                                      </div>
                                      {canUse && (<div className="text-right">
                                          <p className="font-bold text-primary text-sm">
                                            {coupon.discount_type === 'percent'
                                        ? `${coupon.discount_value}%`
                                        : `${coupon.discount_value.toLocaleString()}원`}
                                          </p>
                                          {totalCostBeforeCoupon > 0 && (<p className="text-xs text-muted-foreground">
                            ...
                            {selectedCoupon && couponDiscount > 0 && (<div className="p-2 bg-primary/10 rounded-lg mt-2">
                    ...
                    <div className="space-y-2" ref={totalSummaryRef}>
                      ...
                      {couponDiscount > 0 && (<div className="flex items-center justify-between mt-1">
            ...
            {/* Confirm Button */}
            <Button ref={isTutorial && step === 1 ? paymentButtonRef : null} onClick={handleConfirmStep1} className="w-full h-14 text-lg font-semibold rounded-xl" disabled={isLoading}>
              확인
            </Button>
            
            {/* Step 1 튜토리얼 가이드 */}
            {showGifticonGuide && isTutorial && step === 1 && gifticonGuideRef.current && (<TutorialGuide targetElement={gifticonGuideRef.current} message="기프티콘을 선택해주세요!" position="bottom" onNext={() => {
                    setShowGifticonGuide(false);
        ...
        {showPaymentGuide && isTutorial && step === 1 && paymentButtonRef.current && (<TutorialGuide targetElement={paymentButtonRef.current} message="확인 버튼을 눌러 결제를 진행하세요!" position="top" onNext={() => {
        ...
        {showGifticonGuide && isTutorial && step === 1 && gifticonGuideRef.current && (<TutorialPointer targetElement={gifticonGuideRef.current}/>)}

        {/* 튜토리얼용 터치 유도 원형 포인터 (확인 버튼 유도) */}
        {showPaymentGuide && showConfirmPointer && isTutorial && step === 1 && paymentButtonRef.current && (<TutorialPointer targetElement={paymentButtonRef.current}/>)}
          </>) : step === 2 ? (<>
            ...
            <div className="space-y-4">
              <Card className="p-6">
                ...
                if (!gifticon)
                    return null;
                return (<div key={selected.id} className="flex justify-between items-center bg-muted/30 rounded-lg px-3 py-2">
              ...
              {/* 결제위젯 UI 렌더링 영역 */}
              {!isWidgetRendered && (<div className="flex flex-col items-center justify-center py-12 space-y-4">
              ...
              {/* 결제하기 버튼 */}
              <Button id="payment-button" ref={isTutorial && step === 2 ? paymentButtonRef : null} onClick={async () => {
            ...
            }} className="w-full h-14 text-lg font-semibold rounded-xl" disabled={isLoading || !paymentWidgets}>
                {isLoading ? '처리 중...' : paymentWidgets ? '결제하기' : '준비 중...'}
              </Button>
              
              {/* Step 2 튜토리얼 가이드 */}
              {showPaymentGuide && isTutorial && step === 2 && paymentButtonRef.current && (<TutorialGuide targetElement={paymentButtonRef.current} message="결제하기 버튼을 눌러주세요!" position="top" onNext={() => {
                    setShowPaymentGuide(false);
                    setTutorialStep(null);
                }} onSkip={() => {
                    setShowPaymentGuide(false);
            ...
            <div className="absolute left-2 top-4 flex flex-col gap-3 z-50">
              <Button variant="ghost" size="icon" className="rounded-full" onClick={handleBackFromStep3}>
                <ArrowLeft className="w-5 h-5"/>
              </Button>
              
              <div className="flex flex-col gap-1.5">
                {Array.from({ length: totalCards }).map((_, index) => (<div key={index} className={`w-1 rounded-full transition-all duration-300 ${index === currentCardIndex
                    ? "h-8 bg-primary"
                    : "h-4 bg-muted"}`}/>))}
              </div>
            </div>
            ...
            <div className="flex flex-col h-full py-4 overflow-hidden">
              <div className="flex-1 overflow-hidden snap-y snap-mandatory scrollbar-hide pb-32" style={{
                ...
                return (<div key={`gifticon-${item.id}-${index}`} className="snap-start mb-4" style={{
                ...
                <div ref={membershipCardRef} className="snap-start" style={{
                scrollSnapAlign: 'start',
                scrollSnapStop: 'always',
            }}>
                  <Card className="p-4 rounded-2xl border-border/50 relative overflow-hidden">
                    ...
                    <div className="absolute top-0 left-0 right-0 z-20">
                      <Button ref={membershipAppRef} variant="default" className="w-full h-10 rounded-t-2xl rounded-b-none bg-primary text-primary-foreground flex items-center justify-start gap-2 px-4 text-[11px] font-bold hover:bg-primary/90 transition-colors shadow-sm" onClick={handleMembershipAppCallout}>
                    ...
                    {/* 추후 서비스 예정 오버레이 */}
                    <div ref={membershipBarcodeRef} className="absolute inset-x-0 bottom-0 top-10 flex flex-col items-center justify-center z-10 bg-background/80 rounded-b-2xl">
                      {membershipBarcodeImage ? (<div className="w-full flex-1 flex items-center justify-center p-4">
                          <img src={membershipBarcodeImage} alt="Uploaded Membership Barcode" className="max-w-full max-h-full object-contain rounded-lg shadow-sm"/>
                          <button className="absolute bottom-3 right-3 w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center hover:bg-primary/30 transition-colors" onClick={triggerMembershipUpload}>
                            <Plus className="w-4 h-4 text-primary"/>
                          </button>
                        </div>) : (<>
                          <button className="w-14 h-14 rounded-full bg-primary/10 flex items-center justify-center mb-3 hover:bg-primary/20 transition-all border-2 border-primary/20 border-dashed" onClick={triggerMembershipUpload}>
                    ...
                    {isMembershipModalOpen && (<div className="absolute inset-0 z-30 flex flex-col items-center justify-center rounded-2xl bg-background/90 pt-10 px-4 space-y-3 text-center">
                        <p className="text-sm font-semibold text-muted-foreground">
                          바코드 캡처하시고 stan에서 바로 적립받으세요
                        </p>
                        <button className="w-14 h-14 rounded-full bg-primary/10 flex items-center justify-center mb-0 hover:bg-primary/20 transition-all border-2 border-primary/20 border-dashed" onClick={handleConfirmMembershipLaunch}>
                          <span className="text-xs font-bold text-primary">예</span>
                        </button>
                        <button className="text-xs text-muted-foreground underline" onClick={handleCloseMembershipModal}>
                          아니오
                        </button>
                      </div>)}
            ...
            <div className="absolute bottom-4 left-4 right-4 space-y-3">
              <div className="relative">
                ...
                <Button ref={payAppButtonRef} onClick={handlePayWithNaverPay} className="w-full h-14 text-lg font-semibold rounded-xl opacity-50" disabled={false} // 튜토리얼 중에는 클릭이 가능하도록 활성화
        >
                  {isLoading ? "처리 중..." : "결제앱 실행"}
                </Button>
              </div>
              <Button ref={paymentCompleteButtonRef} onClick={handlePaymentComplete} className="w-full h-14 text-lg font-semibold rounded-xl">
                결제 완료
              </Button>
            </div>
          </>)}
        ...
        {/* Step 3 튜토리얼 가이드 */}
        {showBarcodeGuide && isTutorial && step === 3 && barcodeRef.current && (<TutorialGuide targetElement={barcodeRef.current} message={`바코드를 확인하세요!
화면 어디든 터치하여 다음단계로 이동`} position="top" allowAnywhereClick={true} onNext={() => {
                setShowBarcodeGuide(false);
                setShowMembershipGuide(true);
            }}/>)}

        {showMembershipGuide && isTutorial && step === 3 && membershipAppRef.current && (<TutorialGuide targetElement={membershipAppRef.current} message="멤버십 앱을 실행하여 포인트를 적립하실 수 있습니다!" position="bottom" allowAnywhereClick={false} onNext={() => {
                setShowMembershipGuide(false);
                setShowMembershipBarcodeGuide(true);
            }}/>)}

        {showMembershipBarcodeGuide && isTutorial && step === 3 && membershipBarcodeRef.current && (<TutorialGuide targetElement={membershipBarcodeRef.current} message={`멤버십 바코드를 캡처하여 등록하시면
앱 내에서 바코드를 바로 사용하실 수 있습니다`} position="top" allowAnywhereClick={false} onNext={() => {
                setShowMembershipBarcodeGuide(false);
                setShowPayAppGuide(true);
            }}/>)}

        {showPayAppGuide && isTutorial && step === 3 && payAppButtonRef.current && (<TutorialGuide targetElement={payAppButtonRef.current} message={`결제앱을 실행하실 수 있습니다
(삼성페이, 카카오페이 등)`} position="top" allowAnywhereClick={false} onNext={() => {
                setShowPayAppGuide(false);
                setShowPaymentCompleteGuide(true);
            }}/>)}

        {showPaymentCompleteGuide && isTutorial && step === 3 && paymentCompleteButtonRef.current && (<TutorialGuide targetElement={paymentCompleteButtonRef.current} message="결제 완료 버튼을 눌러 튜토리얼을 마무리하세요!" position="top" allowAnywhereClick={false} onNext={() => {
                setShowPaymentCompleteGuide(false);
                handlePaymentComplete();
            }}/>)}

        {/* 튜토리얼용 터치 유도 원형 포인터 (멤버십 앱 유도) */}
        {showMembershipGuide && isTutorial && step === 3 && membershipAppRef.current && (<TutorialPointer targetElement={membershipAppRef.current}/>)}

        {showMembershipBarcodeGuide && isTutorial && step === 3 && membershipBarcodeRef.current && (<TutorialPointer targetElement={membershipBarcodeRef.current}/>)}

        {showPayAppGuide && isTutorial && step === 3 && payAppButtonRef.current && (<TutorialPointer targetElement={payAppButtonRef.current}/>)}

        {/* 튜토리얼용 터치 유도 원형 포인터 (결제 완료 유도) */}
        {showPaymentCompleteGuide && isTutorial && step === 3 && paymentCompleteButtonRef.current && (<TutorialPointer targetElement={paymentCompleteButtonRef.current}/>)}

        {/* 튜토리얼 완료 모달 */}
        <Dialog open={isTutorialCompleteModalOpen} onOpenChange={setIsTutorialCompleteModalOpen}>
          <DialogContent className="sm:max-w-md rounded-2xl [&>button]:hidden" onPointerDownOutside={(e) => e.preventDefault()} onEscapeKeyDown={(e) => e.preventDefault()}>
            <DialogHeader>
